...
class Oxford102Flowers(Dataset):
    def __init__(self, root: str, split: str = 'train', transform: Optional[Callable] = None, download: bool = True):
        self.root = root
        self.split = split
    ...
    def _download(self):
        ...
        target_path = os.path.join(self.root, "102flowers.tgz")
        extract_path = os.path.join(self.root, "102flowers")

        if not os.path.exists(target_path):
            download_url(url, self.root, "102flowers.tgz")

        if not os.path.exists(extract_path):
            extract_archive(target_path, extract_path)

        url = "https://www.robots.ox.ac.uk/~vgg/data/flowers/102/imagelabels.mat"
        download_url(url, self.root, "imagelabels.mat")

        url = "https://www.robots.ox.ac.uk/~vgg/data/flowers/102/setid.mat"
        download_url(url, self.root, "setid.mat")

    def _load_split(self):
        import scipy.io

        labels_path = os.path.join(self.root, 'imagelabels.mat')
        split_path = os.path.join(self.root, 'setid.mat')
        images_path = os.path.join(self.root, '102flowers', 'jpg')
        ...
        self.images = [os.path.join(images_path, f'image_{i:05d}.jpg') for i in ids]
        self.labels = [labels[i - 1] - 1 for i in ids]  # labels are 1-based
...
def prepare_datasets(
    ...
    if dataset ==  "oxford102flowers":
        DatasetWithIndex = dataset_with_index(Oxford102Flowers)
...
def prepare_datasets_linear(
    ...
    assert dataset in ["cifar10", "cifar100", "stl10", "imagenet", "imagenet100", "pathmnist", "stanford_cars", "cub200", "oxford102flowers", "fgvc_aircraft", "custom"]
