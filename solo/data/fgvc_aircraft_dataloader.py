...
class FGVCAircraft(Dataset):
    def __init__(self, root: str, split: str, transform: Optional[Callable] = None):
        ...
        images_file = os.path.join(root, 'data', 'images_variant_trainval.txt' if split == 'train' else 'images_variant_test.txt')
        self.image_paths = []
        self.labels = []
        ...
        with open(images_file, 'r') as f:
            lines = f.readlines()
            for line in lines:
                image, label = line.strip().split(' ', 1)
                self.image_paths.append(os.path.join(root, 'data', 'images', image + '.jpg'))
...
def prepare_datasets(
    ...
    if dataset ==  "fgvc_aircraft":
        DatasetWithIndex = dataset_with_index(FGVCAircraft)
