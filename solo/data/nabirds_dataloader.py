...
class NABirds(Dataset):
    def __init__(self, root: str, split: str, transform: Optional[Callable] = None):
        ...
        self.image_paths, self.labels = self.load_split_info()

    def load_split_info(self) -> Tuple[List[str], List[int]]:
        ...
        # Read images.txt to map image IDs to file paths
        image_id_to_path = {}
        ...
        for line in split_lines:
            ...
            split_dict[image_id] = int(is_train)

        labels_dict = {}
        for line in labels_lines:
            image_id, label = line.strip().split()
            labels_dict[image_id] = int(label) - 1  # Convert to 0-based index

        image_paths = []
        labels = []
        for image_id, is_train in split_dict.items():
            if (self.split == 'train' and is_train) or (self.split == 'test' and not is_train):
                if image_id in image_id_to_path:
                    image_path = os.path.join(self.images_dir, image_id_to_path[image_id])
                    image_paths.append(image_path)
                    labels.append(labels_dict[image_id])
                else:
                    print(f"Warning: {image_id} not found in images.txt")
...
def prepare_datasets(
    ...
    if dataset == "nabirds":
        train_dataset = dataset_with_index(NABirds)(
            root = train_data_path,
            split = 'train',
            transform = transform,
        )
...
def prepare_datasets_linear(
    ...
    assert dataset in ["cifar10", "cifar100", "stl10", "imagenet", "imagenet100", "pathmnist", "stanford_cars", "cub200", "oxford102flowers", "fgvc_aircraft", "nabirds", "custom"]
